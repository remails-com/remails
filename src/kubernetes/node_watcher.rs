...
use crate::kubernetes::{Error, Kubernetes};
...
impl Kubernetes {
    pub fn spawn_node_watcher(&self, shutdown: CancellationToken) -> Result<JoinHandle<()>, Error> {
        ...
        Ok(tokio::spawn(async move {
            info!("Watching K8s nodes for changes");
            let mut obs = pin!(obs);
            ...
            loop {
                match obs.try_next().await {
                    ...
                    Ok(None) => {
                        ...
                        if error_count > 10 {
                            error!("K8s API watcher stream ended too many times, shutting down");
                            shutdown.cancel();
                            break;
                        }
                        tokio::time::sleep(std::time::Duration::from_secs(5 * error_count)).await;
                    }
                    Err(e) => {
                        error_count += 1;
                        ...
                        if error_count > 10 {
                            error!(
                                "K8s API watcher stream encountered an error too many times, shutting down"
                            );
                            shutdown.cancel();
                            break;
                        }
    ...
    async fn check_for_node_failures(&self, node: Node) -> Result<(), Error> {
