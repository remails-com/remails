...
async fn main() -> anyhow::Result<()> {
    ...
    let block_min_attempts: i64 = std::env::var("BLOCK_MIN_ATTEMPTS")
        .context("BLOCK_MIN_ATTEMPTS must be set")?
        .parse()
        .context("BLOCK_MIN_ATTEMPTS must be a valid integer")?;
    let block_delivery_rate_threshold: f64 = std::env::var("BLOCK_DELIVERY_RATE_THRESHOLD")
        .context("BLOCK_DELIVERY_RATE_THRESHOLD must be set")?
        .parse()
        .context("BLOCK_DELIVERY_RATE_THRESHOLD must be a valid float")?;
