...
import { DeliveryStatus, EmailMetadata, DeliveryDetails, DeliveryDetailsWithRecipient } from "../../types";
...
export function Recipients({ email, mr, ml }: RecipientsProps): ReactElement {
  ...
  const [details, setDetails] = useState<DeliveryDetailsWithRecipient>({ log: { lines: [] }, status: { type: "NotSent" }, recipient: "@" });

  const badges = email.recipients.map((recipient: string) => {
    const details = email.delivery_details[recipient];
    return (
      <Recipient details={details} recipient={recipient} key={recipient} props={{
        ml,
        mr,
        onClick: (e: MouseEvent) => {
          e.stopPropagation();
          if (details?.log) {
            setDetails({ ...details, recipient });
            open();
          }
        },
  ...
  return (
    <>
      <ConnectionLog details={details} opened={opened} close={close} />
      {badges}
    </>
  );
}
