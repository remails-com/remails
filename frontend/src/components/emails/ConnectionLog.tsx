...
import { DeliveryDetailsWithRecipient } from "../../types.ts";
import { Recipient } from "./Recipients.tsx";

interface ConnectionLogProps {
  details: DeliveryDetailsWithRecipient;
  opened: boolean;
  close: () => void;
}
...
export function ConnectionLog({ details, close, opened }: ConnectionLogProps) {
  return (
    <Modal opened={opened} onClose={close} title={
      <Title order={3} component="span">
        Connection log
        <Recipient details={details} recipient={details.recipient} props={{ ml: "xl" }} />
      </ Title>
